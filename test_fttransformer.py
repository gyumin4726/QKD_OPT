"""
FT-Transformer 모델 테스트 (파이프라인 5단계)

학습된 FT-Transformer 모델을 평가하고 예측 성능을 분석하는 스크립트입니다.
...
파이프라인:
    ...
    5. test_fttransformer.py  → 모델 평가  ⬅ 현재 파일
...
사용법:
    1. 파일 상단의 L, EPOCHS, BATCH_SIZE 설정 (train_fttransformer.py와 동일하게)
    2. python test_fttransformer.py 실행
"""
...
# 모델 설정 (train_fttransformer.py와 동일하게 설정)
L = 100                    # 거리 (km)
EPOCHS = 500               # 학습 시 사용한 에포크 수
BATCH_SIZE = 64            # 학습 시 사용한 배치 크기

# 데이터 설정 (train_fttransformer.py와 동일하게 설정)
INCLUDE_Y_0 = False        # Y_0를 입력 변수로 포함할지 여부

# 자동 설정되는 경로
MODEL_PATH = f"qkd_fttransformer_L{L}_E{EPOCHS}_B{BATCH_SIZE}.pth"  # 모델 경로
...
def main():
    ...
    print("=" * 90)
    print(f"QKD FT-Transformer 모델 테스트 (L={L}km)")
