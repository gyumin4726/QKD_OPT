...
def generate_pinn_data(solution):
    ...
    # PINN의 입력으로 사용할 관측값들 (예시)
    E_mu = m_mu_Z / n_mu_Z if n_mu_Z != 0 else 0
    E_nu = m_nu_Z / n_nu_Z if n_nu_Z != 0 else 0
    observables = [Q_mu, E_mu, Q_nu, E_nu] # 실제 필요한 관측값들로 구성

    # PINN의 정답지로 사용할 Ground Truth 값들
    ground_truths = [S_0_Z_L_ex, S_1_Z_L_ex, S_1_X_L_ex, T_1_X_U_ex]

    return (observables, ground_truths) # SKR 대신 데이터 쌍을 반환